...
class PHPExcel_Reader_Excel5 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
    ...
    /**
     * Text object reference for drawing
     *
     * @var mixed
     */
    private $textObjRef;
    ...
    /**
     * Base cell for shared formulas/arrays
     *
     * @var string
     */
    private $_baseCell = 'A1';
    ...
    private function readXf()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            if ($this->version == self::XLS_BIFF8) {
                ...
                if (!isset($objStyle->getFill()->startcolorIndex)) {
                    $objStyle->getFill()->startcolorIndex = 0;
                }
                $objStyle->getFill()->startcolorIndex = (0x007F & self::getInt2d($recordData, 18)) >> 0;

                // bit: 13-7; mask: 0x3F80; color index for pattern background
                if (!isset($objStyle->getFill()->endcolorIndex)) {
                    $objStyle->getFill()->endcolorIndex = 0;  
                }
...
//        echo '<br />';
    ...
    //            var_dump($bcBitCount);
                ...
                $tempFilename = tempnam(sys_get_temp_dir(), 'excel_img');
                imagepng($ih, $tempFilename);
                
                $drawing = new PHPExcel_Worksheet_Drawing();
                $drawing->setPath($tempFilename);
                $drawing->setWorksheet($this->phpSheet);
                
                // Schedule temporary file for deletion when PHP exits
        ...
        foreach ($tokens as $token) {
            ...
            switch ($token['name']) {
                ...
                case 'tArray': // array constant
                    ...
                    if (isset($constantArray['size'])) {
                        $additionalData = substr($additionalData, (int)$constantArray['size']); // bite of chunk of additional data
                    }
                    unset($space0, $space1);
                    break;
                case 'tMemArea':
